...
# If you want servos to move during a dry run for quick feedback, set this True.
...
SERVO_B_PIN = 17
STEP_DIR_PIN = 26
STEP_STEP_PIN = 22  # keep stepper STEP on GPIO22 (avoid conflict with NeoPixel grid)
HOME_SWITCH_PIN = 12
LIMIT_MAX_PIN = 35
...
# Servos: initialize only if allowed
servo_a = None
servo_b = None
if (not DRY_RUN) or ALLOW_SERVOS_IN_DRY_RUN:
    try:
    ...
    print("DRY_RUN: servo outputs suppressed (set ALLOW_SERVOS_IN_DRY_RUN=True to enable)" )
...
servos_active = True if servo_a or servo_b else False
...
def set_servo_a(angle):
    ...
    if servo_a:
        try:
            ...
            print('Servo A set error:', e)
    else:
        print(f"DRY_RUN: would set servo A to {angle}°")
...
def set_servo_b(angle):
    ...
    if servo_b:
        try:
            ...
            print('Servo B set error:', e)
    else:
        print(f"DRY_RUN: would set servo B to {angle}°")
...
def home_curtain():
    ...
    if not DRY_RUN:
        ...
        print("DRY_RUN: homing simulated")
        curtain_stage = 0
        curtain_steps = 0
...
def advance_curtain():
    ...
    if not DRY_RUN:
        ...
        # Simulate partial/complete movement and optionally move servos for visual feedback
        if curtain_stage == 0:
            print("DRY_RUN: Stage 1 simulated - curtain at 50%")
            curtain_stage = 1
            if (servo_a or servo_b):
                set_servo_a(45)
                set_servo_b(45)
        elif curtain_stage == 1:
            print("DRY_RUN: Stage 2 simulated - curtain fully open")
            curtain_stage = 2
            if (servo_a or servo_b):
                set_servo_a(90)
                set_servo_b(90)
        else:
            print("DRY_RUN: Curtain already fully open (simulated)")
